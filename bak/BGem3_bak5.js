// BGem3 -- 3d js canvas drawing engine
    ...
    BGem3.CubeMesh = function(obj) {
        ...
        this.faces = [
            [1,4,3,2,[102,45,145,1],true],
            [1,2,6,5,[102,45,145,1],true],
            [2,3,7,6,[102,45,145,1],true],
            [3,4,8,7,[102,45,145,1],true],
            [4,1,5,8,[102,45,145,1],true],
            [5,6,7,8,[102,45,145,1],true]
        ];
        this.makeTransform();
    };
        ...
        */
            ...
            for (var i=0;i<scene.objs.length;i++) {
                for (var j=0;j<scene.objs[i].mesh.faces.length;j++) {
                    var obj = scene.objs[i],
                        face = scene.objs[i].mesh.faces[j],
                        style = scene.objs[i].mesh.faces[j][4],
                        p1 = {}, p2 = {}, p3 = {}, p4 = {},
                        pts = [p1,p2,p3,p4];
                    p1.v3 = obj.transform[face[0]];
                    p1.v2 = obj.vertices2[face[0]];
                    p2.v3 = obj.transform[face[1]];
                    p2.v2 = obj.vertices2[face[1]];
                    p3.v3 = obj.transform[face[2]];
                    p3.v2 = obj.vertices2[face[2]];
                    p4.v3 = obj.transform[face[3]];
                    p4.v2 = obj.vertices2[face[3]];
                    var zInd = 0;
                    for (var k=0;k<pts.length;k++) {
                        zInd += pts[k].v3.z;
                    }
                    var visible = obj.visible;
                    var zObj = {obj:obj, face:face, p1:p1, p2:p2, p3:p3, p4:p4, zInd:zInd, visible:visible, fillStyle:style, cull:false};
                ...
                for (var i=0;i<renderer.zSort.length;i++) {   // loop through faces
                    if (renderer.zSort[i].visible) {   // if face visible
                        ...
                        if (!renderer.zSort[i].cull) {
                            ...
                            ctx.save();
                            ctx.beginPath();
                            ctx.moveTo( pts[0].v2.x, pts[0].v2.y );
                            ctx.lineTo( pts[1].v2.x, pts[1].v2.y );
                            ctx.lineTo( pts[2].v2.x, pts[2].v2.y );
                            ctx.lineTo( pts[3].v2.x, pts[3].v2.y );
                            ctx.lineTo( pts[0].v2.x, pts[0].v2.y );
                            ctx.closePath();
                            // stroke
        ...
        this.overDraw = function(pts) {
            var pts = pts,
                ...
                p3 = pts[2],
                p4 = pts[3],
                x1 = pts[2].v2.x - pts[0].v2.x,
                y1 = pts[2].v2.y - pts[0].v2.y,
                x2 = pts[3].v2.x - pts[1].v2.x,
                y2 = pts[3].v2.y - pts[1].v2.y;
            var m1 = y1/x1,
                m2 = y2/x2,
                b1 = p1.v2.y - m1*p1.v2.x,
                b2 = p2.v2.y - m2*p2.v2.x,
                mx = (b2-b1) / (m1-m2),
                my = m1*mx + b1;
            for (var i=0;i<pts.length;i++) {
                var dist = BGem3.Math.getDist(mx,my,pts[i].v2.x,pts[i].v2.y),
                    angle = BGem3.Math.getAngle(mx,my,pts[i].v2.x,pts[i].v2.y),
                    r = dist * scale;
                pts[i].v2.x = mx + Math.cos(angle) * r;
                pts[i].v2.y = my + Math.sin(angle) * r;
            }
            return pts;
        };
        this.post = function() {};
