// BGem3 -- 3d js canvas drawing engine
    ...
    BGem3.CubeMesh = function(obj) {
        ...
        this.faces = [
            [1,4,3,2,{fill:true,fillStyle:[102,45,145,1]},true],
            [1,2,6,5,{fill:true,fillStyle:[102,45,145,1]},true],
            [2,3,7,6,{fill:true,fillStyle:[102,45,145,1]},true],
            [3,4,8,7,{fill:true,fillStyle:[102,45,145,1]},true],
            [4,1,5,8,{fill:true,fillStyle:[102,45,145,1]},true],
            [5,6,7,8,{fill:true,fillStyle:[102,45,145,1]},true]
        ];
        this.makeTransform();
    };
    ...
    BGem3.AnchorPt = function() {
        ...
        this.vertices = [
            {x:0.1, y:0.1, z:0.1}, // anchor point
            {x:-this.size, y:-this.size, z:1},
            {x:this.size, y:-this.size, z:1},
            {x:this.size, y:this.size, z:1},
            {x:-this.size, y:this.size, z:1}
        ];
        this.faces = [
            [1,2,3,4,{fill:true,fillStyle:[102,45,145,1]},true]
        ];
        this.makeTransform();
    ...
    BGem3.Renderer = function() {
        ...
        */
        ...
        this.zSortObjs = function(cb) {
            ...
            for (var i=0;i<scene.objs.length;i++) {
                var thisObj = {};
                thisObj.obj = scene.objs[i];
                thisObj.sgPosition = scene.objs[i].sgPosition;
                renderer.zSort.push(thisObj);
            }
            renderer.zSort.sort(function(a,b) {
                return b.sgPosition.z - a.sgPosition.z;
            });
        ...
        this.draw = function(cb) {
            if (renderer.skip) {
                ...
                for (var i=0;i<renderer.zSort.length;i++) {   // loop through objects
                    if (renderer.zSort[i].obj.visible) {   // if object is visible
                        renderer.backfaceCull(renderer.zSort[i].obj);   // backface cull
                        for (var j=0;j<renderer.zSort[i].obj.mesh.faces.length;j++) {   // loop through faces
                            if (renderer.zSort[i].obj.mesh.faces[j][6]==true) {   // if face is visible
                                // get normal for shading
                                var normal = BGem3.Math.getNormal(renderer.zSort[i].obj,j),
                                    angle = normal.angle,
                                    shade = 1-(angle/Math.PI),
                                    shine = 1-(angle/Math.PI)*1.2;
                                // get points
                                var v1 = renderer.zSort[i].obj.mesh.faces[j][0],
                                    v2 = renderer.zSort[i].obj.mesh.faces[j][1],
                                    v3 = renderer.zSort[i].obj.mesh.faces[j][2],
                                    v4 = renderer.zSort[i].obj.mesh.faces[j][3],
                                    stroke = renderer.zSort[i].obj.mesh.stroke,
                                    fill = renderer.zSort[i].obj.mesh.faces[j][4].fill,
                                    fillStyle = renderer.zSort[i].obj.mesh.faces[j][4].fillStyle,
                                    pts = [
                                        renderer.zSort[i].obj.vertices2[v1],
                                        renderer.zSort[i].obj.vertices2[v2],
                                        renderer.zSort[i].obj.vertices2[v3],
                                        renderer.zSort[i].obj.vertices2[v4]
                                    ];
        ...
        this.backfaceCull = function(obj) {
            for (var i=0;i<obj.mesh.faces.length;i++) {
                var p1 = obj.mesh.faces[i][0],
                    p2 = obj.mesh.faces[i][1],
                    p3 = obj.mesh.faces[i][2],
                    a = obj.vertices2[p1],
                    b = obj.vertices2[p2],
                    c = obj.vertices2[p3],
                    ax = b.x - a.x,
                    ...
                    z = (ax*by) - (ay*bx);
                if (z<0) {
                    obj.mesh.faces[i][6] = false;
                } else {
                    obj.mesh.faces[i][6] = true;
                }
            }
        };
    ...
    BGem3.MathFunc = function() {
        ...
        this.getNormal = function(obj,face) {
            var light = {x:100,y:-200,z:50};
            var p1 = obj.mesh.faces[face][0],
                p2 = obj.mesh.faces[face][1],
                p3 = obj.mesh.faces[face][2];
            var Ax = obj.transform[p2].x - obj.transform[p1].x,
                Ay = obj.transform[p2].y - obj.transform[p1].y,
                Az = obj.transform[p2].z - obj.transform[p1].z,
                Bx = obj.transform[p3].x - obj.transform[p1].x,
                By = obj.transform[p3].y - obj.transform[p1].y,
                Bz = obj.transform[p2].z - obj.transform[p1].z;
            var Cx = Ay*Bz - Az*By,
                Cy = Az*Bx - Ax*Bz,
                Cz = Ax*By - Ay*Bx;
            var ax = Cx,
