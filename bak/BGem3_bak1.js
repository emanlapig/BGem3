...
BGem3.Scene = function() {
    ...
    this.add = function(obj) {
        obj.translate = obj.vertices3.slice();
        this.objs.push(obj);
    }
        ...
        for (var i=0;i<scene.objs.length;i++) {
            ...
            for (var k=0;k<scene.objs[i].translate.length;k++) {
                var vy = scene.objs[i].translate[k].y,
                    ...
                    angle2 = (lg=='local') ? angle - (scene.objs[i].lRotation.x * Math.PI/180) : angle - (scene.objs[i].gRotation.x * Math.PI/180);
                if (angle2>2*Math.PI) {
                    angle2-=2*Math.PI;
                }
            ...
            for (var j=0;j<scene.objs[i].translate.length;j++) {
                var vx = scene.objs[i].translate[j].x,
                    ...
                    angle2 = (lg=='local') ? angle - (scene.objs[i].lRotation.y * Math.PI/180) : angle - (scene.objs[i].gRotation.y * Math.PI/180);
                if (angle2>2*Math.PI) {
                    angle2-=2*Math.PI;
                }
            ...
            for (var l=0;l<scene.objs[i].translate.length;l++) {
                var vx = scene.objs[i].translate[l].x,
                    ...
                    angle2 = (lg=='local') ? angle - (scene.objs[i].lRotation.z * Math.PI/180) : angle - (scene.objs[i].gRotation.z * Math.PI/180);
                if (angle2>2*Math.PI) {
                    angle2-=2*Math.PI;
                }
    ...
    this.draw = function(cb) {
        ...
        for (var i=0;i<scene.objs.length;i++) {
            for (var j=0;j<scene.objs[i].mesh.faces.length;j++) {
                ...
                //ctx.strokeStyle="rgba("+rand+","+rand+","+rand+",1)";
...
BGem3.RotController = function(obj) {
    ...
    $(this.div).mousedown(function(event) {
        ...
        $(this).mousemove(function(event) {
            ...
            controller.obj3d.lRotation.x -= deltaX/2;
            controller.obj3d.lRotation.z -= deltaY/2;
            //console.log(controller.obj3d.lRotation.x);
            controller.mouseX = event.pageX;
            controller.mouseY = event.pageY;
        }).mouseup(function(event) {
            $(this).unbind('mousemove');
            controller.obj3d.applyLocalRot = true;
        });
    });
}
